...
public class ProductsController {
	...
	// 상품 목록 페이지 - 기본
	@GetMapping
	public String getProductList(@RequestParam(name = "page", defaultValue = "1") Integer page,
		...
		if ("all".equals(category) || "0".equals(category)) {
			category = "0"; // 카테고리가 "all" 또는 "0"일 경우 모든 상품 조회
			...
			count = productsService.countProducts(params);
			products = productsService.getProductsWithPage(params);
		}
		...
		model.addAttribute("category", category); // 카테고리 값
		model.addAttribute("categoryName", categoryName); // 카테고리 이름
		model.addAttribute("keyword", keyword);
		model.addAttribute("sort", sort); // 카테고리 정렬
	...
	public String getProductsShopList(@PathVariable("sellerId") int sellerId,
	    ...
	    List<ProductsDto> products = productsService.findBySellerIdAndPage(params);
	...
	public String getProductsBySellerId(@PathVariable("sellerId") int sellerId,
		...
		int count = productsService.countBySellerId(sellerId); // 해당 sellerId에 맞는 총 상품 수
