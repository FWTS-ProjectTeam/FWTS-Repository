...
//	// ✅ 모든 주문 정보 조회
//    @Select("SELECT o.order_num, o.purchase_quantity, o.total_price, o.order_state, " +
//    		// 주문 번호, 구매 수량, 총 결제액, 주문 및 배송 상태
//    		"p.pro_name " + // 상품명
//    		"FROM orders o " +
//    		"JOIN products p ON o.pro_id = p.pro_id " + // 상품 아이디로 조인
//    		"WHERE o.seller_id = #{sellerId}") // 현재 로그인한 판매자의 아이디로 조회
//    		List<OrderList> getAllOrders(@Param("sellerId") int sellerId);
//   
    // 조인을 통해 필요한 데이터를 한번에 가져올 수 있다면 Map<String, Object>를 사용할 필요 x
    // 만일 Map으로 반환하고 싶다면 List<Map<S, O>>를 사용하면 됨
...
//    @Select("SELECT o.order_num, p.pro_name, o.purchase_quantity, o.total_price, o.order_state " +
//    		"FROM orders o " +
//    		"JOIN products p ON o.pro_id = p.pro_id " +
//    		"WHERE o.seller_id = #{sellerId} " +
//    		"ORDER BY o.order_date DESC " +
//    		"LIMIT #{offset}, #{limit}") // 한번에 10개씩 조회
//    List<OrderList> getPagedOrderList(@Param("sellerId") int sellerId, @Param("offset") int offset, @Param("limit") int limit);
//    
